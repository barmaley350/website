...
После заголовка оставь пустую строку и добавь тело коммита: кратко опиши на русском языке какие именно изменения были сделаны (не более 3–4 строк), зачем это нужно и на что может повлиять. Не используй markdown.
EOF
